...
2. **Adicionar carta** → escolha a carta (puxa os dados padrão do catálogo), defina estado, quantidade, valor, pública/privada e à venda.
3. Na biblioteca, use **Solicitar graduação** numa carta crua → preencha o checklist → gere o protocolo.
...
- **Adicionar carta** — salva na biblioteca com os **dados padrão da carta** (nome, coleção, número,
  raridade, tipo, ano) + **foto da carta** (anexar/arrastar), estado, quantidade, valor,
  **pública/privada** e **à venda (sim/não)**. A foto aparece na biblioteca, na vitrine e no mercado.
- **Minha biblioteca** — cada carta tem dois toggles independentes: *pública* (aparece na vitrine)
  e *à venda* (aparece no mercado). Cartas não graduadas têm o botão **Solicitar graduação**.
- **Graduar pela TAG Grading (parceira)** — leva à página com **checklist obrigatório** (proprietário,
  autêntica, sem alteração, limpa, aceite de termos, embalagem) + valor declarado e serviço; gera um
  **protocolo**, registra o pedido na biblioteca como *Encaminhado à TAG Grading* e **abre o site da
  TAG Grading** (parceira de autenticação no Brasil) numa nova aba para concluir a submissão.

  > A URL do parceiro fica em `PARTNER` no topo do `<script>` de `slabr_app.html`
  > (`https://www.taggrading.com.br/`). Quando a TAG liberar o deep-link de submissão do Brasil,
  > basta trocar essa URL. Não embutimos parâmetros de rastreio (fbclid/utm) no produto.
