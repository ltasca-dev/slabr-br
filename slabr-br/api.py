...
def offers(cid):
    rows=db().execute("SELECT * FROM graded_items WHERE card_id=? AND public=1 ORDER BY declared_value_cents",(cid,)).fetchall()
...
def add_library():
    ...
    db().execute("""INSERT OR REPLACE INTO graded_items
        (cert_id,card_id,vertical,grade,gem,sub_centering,sub_corners,sub_edges,sub_surface,
         scan_url_front,owner_handle,declared_value_cents,graded_at,public,condition,qty,graded)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),?,?,?,?)""",
        (cert, cid, "TCG", grade, 1 if grade==10 else 0, None,None,None,None,
         None, h, value, pub, condition, qty, graded))
    db().commit()
    return jsonify({"ok":True,"cert":cert,"graded":bool(graded),"condition":condition})
...
def market():
    ...
    rows=db().execute("""SELECT g.cert_id,g.grade,g.gem,g.declared_value_cents,g.owner_handle,
                                g.graded,g.condition,
                                c.id cid,c.name cname,c.number cnum,c.types ctypes,c.rarity crar,s.name sname
                         FROM graded_items g JOIN cards c ON c.id=g.card_id JOIN sets s ON s.id=c.set_id
                         WHERE g.public=1 ORDER BY g.declared_value_cents DESC LIMIT ?""",(limit,)).fetchall()
